  ...
  return str.split('').map((char: string) => accents[char] || char).join('');
   ...
   const mostrarSolucionesQuimicas = (index: number) => {
    ...
    useFocusEffect(
      ...
      setMostrarEnf(enfermedadesList.map((_, index) => index === Number(enfermedadId))); // Mostrar solo la enfermedad seleccionada
      ...
      {enfermedades.map((enfermedad,index) => (
        ...
        >
          ...
          {mostrarEnf[index] && (
            ...
            <TouchableOpacity onPress={() => mostrarSolucionesQuimicas(index)} style={{ marginTop: 10 }}>


              <Text style={{ fontSize: 18, fontWeight: 'bold' }}>Soluciones Químicas</Text>
            </TouchableOpacity>

            {mostrarQuimicas[index] && (
              <View>
                <Text style={{ fontSize: 16 }}>{enfermedad.quimicas}  </Text>
                {enfermedad.imgQuimicas && Array.isArray(enfermedad.imgQuimicas) && enfermedad.imgQuimicas.map((imgUrl: string, imgIndex: number) => (
                  <Image
                    key={imgIndex}
                    source={{ uri: imgUrl }}
             ...
             <TouchableOpacity onPress={() => mostrarSolucionesBiologicas(index)} style={{ marginTop: 10 }}>

              <Text style={{ fontSize: 18, fontWeight: 'bold' }}>Soluciones Biológicas</Text>
            </TouchableOpacity>
            {mostrarBiologicas[index] && (
              <View>
              <Text style={{ fontSize: 16 }}>{enfermedad.biologicas}  </Text>
               {enfermedad.imgBiologicas && Array.isArray(enfermedad.imgBiologicas) && enfermedad.imgBiologicas.map((imgUrl: string, imgIndex: number) => (
                  <Image
                    key={imgIndex}
                    source={{ uri: imgUrl }}
                    style={{ width: 70, height: 50 }}

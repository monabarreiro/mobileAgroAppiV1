...
export default function Admin() {
  ...
  const [quimicas, setQuimicas] = useState<string[]>([""]); //Titulo 1 - Marca del Producto
  ...
  const BorrarEnfermedad = async (id: any) => {
    // selectivamente es asyncronico
    try {
  ...
  const BorrarCultivo = async (cultivos: any) => {
    // selectivamente es asyncronico
    try {
  ...
  const aniadirPrimDescripcion = () => {
    setPrimDescripcion([...primDescripcion, ""]);
  };

  const aniadirSegDescripcion = () => {
    setSegDescripcion([...segDescripcion, ""]);
  };
  ...
  const mensajesActuales = mensajes.slice(
    indexPrimerMensaje,
    indexUltimoMensaje,
  );
  ...
  const handlePrimDescripcion = (index: any, event: any) => {
    const newLinks = [...primDescripcion];
    newLinks[index] = event.target.value;
    setPrimDescripcion(newLinks);
  };
  const handleSegDescripcion = (index: any, event: any) => {
    const newLinks = [...segDescripcion];
    newLinks[index] = event.target.value;
    setSegDescripcion(newLinks);
  };
    ...
    try {
      ...
      if (!querySnapshot.empty) {
        ...
        cultivosList.forEach((cultivo) => {
          ...
          setListadoCultivos((ListaCultivos) => [
            ...ListaCultivos,
            cultivo.cultivo ?? "",
          ]);
          setPaginasTotalesCultivos(
            Math.ceil(cultivosList.length / postsPorPaginaCultivos),
          );
        });
        ...
        console.log(
          "No se encontraron documentos en la colección $ {textoFiltrado}",
        );
      }
    } catch (error) {
  ...
  const cultivosActuales = listadoCultivos.slice(
    indexPrimerCultivo,
    indexUltimoCultivo,
  );
  const paginateCultivos = (pageNumber: any) =>
    setPaginaActualCultivos(pageNumber);
  ...
  onAuthStateChanged(auth, (user) => {
    if (user) {
      ...
      // ...
      if (uid != "XLoHa1Ab8lX67SEAjyN4Qe5vjCA2") {
  ...
  return (
    ...
    >
            ...
            <tr>
              ...
              <td>
                ...
                <button
                  ...
                  onClick={() => crearCultivo(cultivo, img)}
                >
                  Crear cultivo
                </button>
              </td>
      ...
      <table className="table">
        ...
        <th scope="col">Marca Soluciones Químicas</th>
        <th scope="col">Descripción Primera Químicas</th>
        <th scope="col">Descripción Segunda Químicas</th>
        ...
        <tbody>
          <tr>
            ...
            <td>
              ...
              {quimicas.map((link, index) => (
                <div key={index}>
                  <input
                    type="url"
                    value={primDescripcion[index]}
                    onChange={(event) => handlePrimDescripcion(index, event)}
                  />
                </div>
              ))}{" "}
            </td>
            ...
            <td>
              ...
              {quimicas.map((link, index) => (
                <div key={index}>
                  <input
                    type="url"
                    value={segDescripcion[index]}
                    onChange={(event) => handleSegDescripcion(index, event)}
                  />
                </div>
              ))}{" "}
            </td>
            <td>
              {" "}
              {imgQuimicas.map((link: any, index: any) => (
                <div key={index}>
                  <input
                    type="url"
                    value={link}
            ...
            <td>
              {" "}
              <button onClick={aniadirLink}>Añadir Fotos</button>
              <button onClick={aniadirQuimicas}>
                Añadir Descripción Químicas{" "}
              </button>
      ...
      <div className="bg-dark">
        ...
        <div className="m-4 ">
            ...
            <tbody>
                  ...
                  <tr key={index}>
                    ...
                    <td>
                      <button
                        ...
                        data-toggle="modal"
                        data-target=".bd-crite-modal-lg"
                        value={idCultivo[index]}
                        onClick={() => {
                          BorrarCultivo(idCultivo[index]);
                        }}
                      >
                        Borrar
                      </button>
              ...
              {paginasTotalesCultivos > 1 ? (
                <div className="pagination">
                  {Array.from({ length: paginasTotalesCultivos }).map(
                    (_, index) => (
                      <button
                        key={index}
                        onClick={() => paginateCultivos(index + 1)}
                      >
                        {index + 1}
                      </button>
                    ),
          ...
          <select
            ...
            id=" "
            value={valor}
            onChange={(event) => CambioDeCultivo(event)}
          >
                  ...
                  <tr key={index}>
                    ...
                    <td>
                      <button
                        ...
                        data-target=".bd-crite-modal-lg"
                        onClick={() => {
                          BorrarEnfermedad(idEnfermedad[index]);
                        }}
                      >
                        Borrar
                      </button>
